...
import { CATEGORIES, DEFAULT_NEGATIVE_PROMPT, TEMPLATES, type Option, type Template } from '../constants';
...
export default function PromptArchitectView() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      try { setSavedPrompts(JSON.parse(stored)); } catch (e) { console.error('Failed to parse saved prompts', e); }
    }
    const storedCustom = localStorage.getItem('prompt_architect_custom_options');
    if (storedCustom) {
      try { setCustomOptions(JSON.parse(storedCustom)); } catch (e) { console.error('Failed to parse custom options', e); }
    }
  }, []);

  const addCustomOption = (categoryId: string, label: string, value: string) => {
    const newOpt: Option = { id: `custom-${crypto.randomUUID()}`, label: label.trim(), value: value.trim() };
    const updated = { ...customOptions, [categoryId]: [...(customOptions[categoryId] || []), newOpt] };
  ...
  const deleteCustomOption = (categoryId: string, optionId: string) => {
    const updated = { ...customOptions, [categoryId]: (customOptions[categoryId] || []).filter(o => o.id !== optionId) };
    setCustomOptions(updated);
    localStorage.setItem('prompt_architect_custom_options', JSON.stringify(updated));
    if (selections[categoryId] === optionId) setSelections(prev => ({ ...prev, [categoryId]: '' }));
  };

  const savePrompt = () => {
    if (!generatedPrompt.trim()) return;
    const newPrompt: SavedPrompt = { id: crypto.randomUUID(), positive: generatedPrompt, negative: negativePrompt, timestamp: Date.now() };
    const updated = [newPrompt, ...savedPrompts].slice(0, 10);
  ...
  const handleSelect = (categoryId: string, optionId: string) => {
    setSelections(prev => ({ ...prev, [categoryId]: prev[categoryId] === optionId ? '' : optionId }));
  };
  ...
  const generatedPrompt = useMemo(() => {
    ...
    return parts.join(', ');
  }, [subject, selections, mode, customOptions]);
    ...
    <main className="flex-1 max-w-[1400px] mx-auto w-full p-4 md:p-8 flex flex-col">
      ...
      <div className="mb-12 text-center md:text-left">
        ...
        <div className="flex flex-wrap gap-3 justify-center md:justify-start">
          ...
          {TEMPLATES.map((t) => (
            <button key={t.id} onClick={() => loadTemplate(t)} className="px-4 py-2 bg-[#151619] hover:bg-[#2A2B2F] border border-white/5 rounded-full text-[10px] font-mono text-gray-400 hover:text-[#FF6B00] transition-all">
              {t.name}
            </button>
          ))}
        </div>
          ...
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-4">
              ...
              <div className="flex bg-[#2A2B2F] rounded-full p-1 border border-white/5">
                <button onClick={() => setMode('image')} className={`px-4 py-1.5 rounded-full text-[10px] font-mono transition-all ${mode === 'image' ? 'bg-[#FF6B00] text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}>IMAGE</button>
                <button onClick={() => setMode('video')} className={`px-4 py-1.5 rounded-full text-[10px] font-mono transition-all ${mode === 'video' ? 'bg-[#FF6B00] text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}>VIDEO</button>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between mb-2">
                ...
                <button onClick={reset} className="text-[10px] font-mono text-gray-500 hover:text-[#FF6B00] transition-colors flex items-center gap-1 uppercase tracking-wider">
              ...
              {CATEGORIES.map((cat) => {
                ...
                return (
                  <div key={cat.id} className="border border-white/5 rounded-xl overflow-hidden bg-[#1D1E22]">
                    <button onClick={() => setExpandedCategory(isExpanded ? null : cat.id)} className="w-full flex items-center justify-between p-4 hover:bg-white/5 transition-colors">
                    ...
                    <AnimatePresence>
                      {isExpanded && (
                        <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="">
                          <div className="p-4 pt-0 grid grid-cols-1 lg:grid-cols-2 gap-2">
                            {[...cat.options, ...(customOptions[cat.id] || [])].map((opt) => (
                              <button
                                ...
                                className={`relative group flex items-center justify-between p-3 rounded-lg text-xs transition-all border ${selectedOption === opt.id ? 'bg-[#FF6B00]/10 border-[#FF6B00]/30 text-white' : 'bg-[#2A2B2F]/50 border-white/5 text-gray-400 hover:border-white/20'}`}
                              >
                                <div className="flex items-center gap-2">
                                  <span>{opt.label}</span>
                                  {opt.id.startsWith('custom-') && <span className="text-[8px] px-1 py-0.5 bg-white/5 rounded text-gray-500 uppercase">Custom</span>}
                                </div>
                                <div className="flex items-center gap-2">
                                  {opt.id.startsWith('custom-') && (
                                    <button onClick={(e) => { e.stopPropagation(); deleteCustomOption(cat.id, opt.id); }} className="p-1 text-gray-600 hover:text-[#FF6B00] transition-colors opacity-0 group-hover:opacity-100">
            ...
            <div className="mb-8">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-2">
                  <button onClick={reset} className="p-2 text-gray-500 hover:text-[#FF6B00] transition-colors" title="Reset All">
              ...
              <div className="relative group">
                ...
                <div className="relative bg-[#151619] border border-white/10 rounded-2xl p-6 min-h-[180px] flex flex-col">
                  ...
                  <div className="mt-auto pt-6 flex justify-end gap-3">
                    <button disabled={!generatedPrompt} onClick={exportToTxt} className={`flex items-center gap-2 px-6 py-2.5 rounded-full text-xs font-mono transition-all ${!generatedPrompt ? 'bg-gray-800 text-gray-600 cursor-not-allowed' : 'bg-[#2A2B2F] text-white hover:bg-[#3A3B3F] active:scale-95 border border-white/5'}`} title="Export as .txt">
                      <Download className="w-4 h-4" /> EXPORT
                    </button>
                    <button disabled={!generatedPrompt} onClick={savePrompt} className={`flex items-center gap-2 px-6 py-2.5 rounded-full text-xs font-mono transition-all ${!generatedPrompt ? 'bg-gray-800 text-gray-600 cursor-not-allowed' : saved ? 'bg-blue-500 text-white' : 'bg-[#2A2B2F] text-white hover:bg-[#3A3B3F] active:scale-95 border border-white/5'}`}>
                      {saved ? <Check className="w-4 h-4" /> : <Save className="w-4 h-4" />}
                      {saved ? 'SAVED' : 'SAVE'}
                    </button>
                    <button disabled={!generatedPrompt} onClick={() => copyToClipboard(generatedPrompt)} className={`flex items-center gap-2 px-6 py-2.5 rounded-full text-xs font-mono transition-all ${!generatedPrompt ? 'bg-gray-800 text-gray-600 cursor-not-allowed' : copied ? 'bg-green-500 text-white' : 'bg-[#FF6B00] text-white hover:bg-[#FF8533] active:scale-95'}`}>
            ...
            <div className="mb-8">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-3">
                  <button onClick={() => setNegativePrompt('')} className="text-[10px] font-mono text-gray-500 hover:text-[#FF6B00] transition-colors flex items-center gap-1 uppercase tracking-wider">
                    <Trash2 className="w-3 h-3" /> Clear
                  </button>
                  <button onClick={() => setNegativePrompt(DEFAULT_NEGATIVE_PROMPT)} className="text-[10px] font-mono text-gray-500 hover:text-[#FF6B00] transition-colors flex items-center gap-1 uppercase tracking-wider">
                    <RotateCcw className="w-3 h-3" /> Reset Default
                  </button>
                </div>
              </div>
              <div className="bg-[#151619] border border-white/5 rounded-2xl p-6">
                <textarea value={negativePrompt} onChange={(e) => setNegativePrompt(e.target.value)} className="w-full bg-transparent border-none text-sm text-gray-400 focus:outline-none resize-none h-24 custom-scrollbar" />
                <div className="mt-4 flex justify-end">
                  <button onClick={() => copyToClipboard(negativePrompt)} className="text-[10px] font-mono text-gray-500 hover:text-white transition-colors flex items-center gap-1">
                    ...
                    {savedPrompts.map((p) => (
                      <div key={p.id} className="group flex items-center justify-between gap-3 p-2 bg-[#151619] rounded-lg border border-white/5 hover:border-white/10 transition-colors">
                        ...
                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => copyToClipboard(p.positive)} className="p-1 text-gray-500 hover:text-white transition-colors" title="Copy Positive">
                            <Copy className="w-3 h-3" />
                          </button>
                          <button onClick={() => deleteSavedPrompt(p.id)} className="p-1 text-gray-500 hover:text-[#FF6B00] transition-colors" title="Delete">
                            <Trash2 className="w-3 h-3" />
                          </button>
                        </div>
                      </div>
  ...
  return (
    <div className="mt-2 p-3 bg-[#151619]/50 rounded-lg border border-dashed border-white/10">
      ...
      <div className="space-y-2">
        <input type="text" placeholder="Label (e.g. Tim Burton Style)" value={label} onChange={(e) => setLabel(e.target.value)} className="w-full bg-[#2A2B2F] border border-white/5 rounded-md px-3 py-1.5 text-[10px] text-white focus:outline-none focus:border-[#FF6B00]/30" />
        <textarea placeholder="Prompt Value (e.g. dark, whimsical, gothic...)" value={value} onChange={(e) => setValue(e.target.value)} className="w-full bg-[#2A2B2F] border border-white/5 rounded-md px-3 py-1.5 text-[10px] text-white focus:outline-none focus:border-[#FF6B00]/30 h-20 resize-none custom-scrollbar" />
        {(label || value) && (
          <div className="p-2 bg-[#0A0A0B] rounded border border-white/5">
            ...
            <p className="mt-1 text-[9px] text-gray-500 italic line-clamp-2 px-1">{value || 'No value defined...'}</p>
          </div>
        )}
        <button onClick={handleSubmit} disabled={!label.trim() || !value.trim()} className={`w-full text-[10px] font-mono py-1.5 rounded-md transition-colors border ${!label.trim() || !value.trim() ? 'bg-gray-800/50 text-gray-600 border-white/5 cursor-not-allowed' : 'bg-[#FF6B00]/10 hover:bg-[#FF6B00]/20 text-[#FF6B00] border-[#FF6B00]/20'}`}>
          + ADD OPTION
        </button>
      </div>
    </div>
